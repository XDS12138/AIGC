...
Decoder-only 由多层解码器块堆叠，核心是因果（Causal）self-attention。每个 token 的表示严格基于历史上下文，符合 $h_t=f(x_{\le t})$ 的信息依赖。
...
如果你愿意，我也可以把你这份《大模型基础概念与原理.md》里**BPE 那一节和这一节的风格统一**，顺手帮你把“训练范式（预训练/指令微调/对齐）+ 增强机制（RAG/工具）”接成下一小节，保证整章阅读连贯。

---

如果你后续要把这一节和你自己的方向（比如水下/海洋感知、多模态雷达-运动耦合、声呐 SLAM 等）对齐，我可以帮你把“结构选择理由 + RAG/工具增强落点”写成同样 **可直接贴 MD/论文** 的一段连续叙述。
