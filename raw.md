...
<h1 id='83.什么是VAPO算法？'>83.什么是VAPO算法？</h1>
...
<h1 id='84.Muon优化器介绍'>84.Muon优化器介绍</h1>

Muon 是一种针对神经网络隐藏层参数设计的革命性优化器，由研究者 Keller Jordan 于 2024 年 12 月通过博客文章首次提出。它通过独特的正交化更新机制显著提升了训练效率，已在多个基准测试中刷新记录，并应用于万亿参数模型（如 Kimi K2）的训练中。

---

### ⚙️ **一、核心技术原理与创新**
1. **牛顿-舒尔茨正交化（Newton-Schulz Orthogonalization）**  
   Muon 的核心是对 SGD 动量更新矩阵进行正交化处理。传统优化器（如 AdamW）的更新矩阵常呈现病态条件数（奇异值差异达 10³ 倍），导致少数方向主导更新。Muon 通过 5 步牛顿-舒尔茨迭代，将动量矩阵 \( M_t \) 逼近其最近的正交矩阵 \( UV^\top \)：
   ```python
   ...
   ```
   - **数学本质**：最小化 Frobenius 范数 \( \min_O \|O - G\|_F \) 且满足 \( O^\top O = I \)，等价于 SVD 分解后的 \( UV^\top \) 。
   - **系数调优**：系数 \( (3.4445, -4.7750, 2.0315) \) 通过优化收敛速度确定，确保 5 步迭代后误差 \( \varepsilon < 0.3 \) 。

2. **对比 SVD 的优势**  
   - 计算效率比 SVD 高 10 倍以上，且支持 bfloat16 精度下的稳定运行。
   - 完美匹配 GPU 的 SIMD 架构，Tensor Core 利用率达 95%+ 。

---

### ⚡ **二、性能优势与实证结果**
Muon 在多个任务中刷新训练速度记录，同时保持极低额外开销（约 1%）：

| **任务场景**         | **性能提升**                                                                 | **技术意义**                     |
|----------------------|-----------------------------------------------------------------------------|----------------------------------|
| **CIFAR-10**         | 94% 准确率耗时从 3.3 A100 秒降至 2.6 A100 秒                                | 加速 21%  |
| **NanoGPT**          | 验证损失 3.28 的耗时降低 1.35 倍                                            | 样本效率与墙钟时间双优  |
| **15 亿参数模型**    | 达 GPT-2 XL 性能仅需 10 小时（8×H100），比 AdamW 节省 25% 时间  | 证明大模型扩展性                 |
| **万亿模型稳定性**   | 在 Kimi K2 中结合 MuonClip 技术，15.5 万亿 token 训练零损失尖峰  | 解决注意力 logits 爆炸问题       |
...
1. **参数适用范围**  
   - **适用**：全连接层权重（2D 矩阵）。
   - **不适用**：向量参数（如 LayerNorm 的 gamma）或稀疏参数（如 Embedding 层）。

2. **混合优化策略**  
   建议输入/输出层使用 AdamW，隐藏层使用 Muon，以平衡效率与稳定性：
   ```python
   # PyTorch 示例
   optimizer = torch.optim.AdamW([
       {'params': model.embeddings},    # 输入层
       {'params': model.hidden_layers, 'optimizer': Muon()},  # 隐藏层
       {'params': model.head}           # 输出层
   ])
   ```

<h1 id='85.QK-Clip技术介绍'>85.QK-Clip技术介绍</h1>

Qk-Clip（或称QK-Clip）是一种针对大规模神经网络（尤其是Transformer架构）训练过程中“MaxLogit爆炸”问题的创新优化技术，由苏剑林团队提出，并成功应用于万亿参数模型**Kimi K2**的训练中。

---

### ⚙️ **一、问题背景：MaxLogit爆炸**
在Transformer的注意力机制中，**MaxLogit** 指Softmax前的注意力矩阵（即 \( QK^{\top} \)）的最大值：
$$
S_{\max} = \max_{i,j} \, \boldsymbol{q}_i \cdot \boldsymbol{k}_j
$$
**问题本质**：  
- 当模型规模扩大（如千亿参数以上），\( S_{\max} \) 可能随训练线性或超线性增长，超出可控范围。  
- 根源在于 \( Q \) 和 \( K \) 矩阵的谱范数（\( \|W_q\|, \|W_k\| \)）异常增大，导致注意力计算出现极端异常值。  
- **风险**：可能引发梯度尖峰（Grad Spike）或训练崩溃，即使Softmax能压缩数值，异常头（Attention Head）仍可能失效。

**传统方法局限**：  
- **Weight Decay**：对小模型有效，但大模型中需大幅增加强度，导致效果损失。  
- **SoftCap（Gemma2）**：通过 \( \tau \tanh(x/\tau) \) 限制Logit值，但仅约束输出而非源头，治标不治本。  
- **QK-Norm（Gemma3）**：对 \( Q, K \) 单独归一化，但**无法兼容MLA（Multi-Head Latent Attention）架构**，因Decoding阶段无法Materialize完整 \( K \) 矩阵。

---

### ⚡ **二、Qk-Clip技术原理**
Qk-Clip的核心思想是**直接监控MaxLogit信号，动态裁剪权重以源头抑制异常值**，实现“精准干预，最小扰动”。

#### **1. 基础操作流程**
- **监控**：实时追踪每个注意力头的 \( S_{\max} \)。  
- **触发**：当 \( S_{\max} > \tau \)（阈值，如100）时，对引发异常的 \( Q/K \) 权重进行缩放。  
- **裁剪**：若第 \( l \) 层第 \( h \) 头的MaxLogit超阈值，则更新其权重：
  $$
  W_{q}^{(l,h)} \leftarrow W_{q}^{(l,h)} \cdot \frac{\tau}{S_{\max}^{(l,h)}}
  $$
  通过缩放 \( W_q \) 直接降低后续计算的Logit值。

#### **2. 关键创新设计**
- **Per-Head裁剪**：  
  早期版本对整层所有头统一裁剪，导致正常头被“误伤”，权重趋于零（过度裁剪）。改进后**独立监控每个头**，仅对异常头操作，避免牵连。  
- **MLA兼容性优化**：  
  针对MLA中 \( Q, K \) 分解为多部分（如qr, qc, kr, kc）的特性，**仅裁剪qr部分**（因kr共享，裁剪会波及无关头），确保兼容性与精度。  
- **自动停机制**：  
  当所有头 \( S_{\max} < \tau \) 时，Qk-Clip自动休眠，减少干预。

#### **3. 数学本质**
通过权重缩放等效于对Logit的事前约束：
$$
\boldsymbol{q}_i \cdot \boldsymbol{k}_j \rightarrow \left(\boldsymbol{q}_i \cdot \frac{\tau}{S_{\max}}\right) \cdot \boldsymbol{k}_j \leq \tau
$$
直接保证裁剪后MaxLogit不超过阈值。

<h1 id='86.什么是预训练Scaling法则？'>86.什么是预训练Scaling法则？</h1>

- 论文原文：[Training Compute-Optimal Large Language Models](https://arxiv.org/abs/2203.15556)

在给定计算预算下训练大型语言模型时，模型大小（参数量）与训练token数量应遵循等比例缩放法则：**模型规模每翻倍，所需训练token数量也需同步翻倍**。这一结论通过训练400多个不同规模的语言模型（参数量从7000万到160亿，训练token从50亿到5000亿）得到验证，并修正了Kaplan等人先前提出的模型规模增长应显著快于数据增长的认知。**证明了通过扩大训练数据集规模、增加模型参数数量和计算资源，可以实现模型性能的提升**。

<h1 id='87.什么是后训练Scaling法则？'>87.什么是后训练Scaling法则？</h1>

- 论文原文：[Scaling Laws for Post Training Quantized Large Language Models](https://arxiv.org/abs/2410.12119)

后训练技术可以进一步提升预训练模型的计算效率、准确率和领域适配性。其规律包括以下4点：

（1）**局部损失的缩放规律**：量化可视为对预训练权重的扰动，量化后损失受局部损失特性影响。同一 LLM 家族中，更大的模型具有更平坦的局部损失，面对相同扰动时损失变化更小，且这种平坦性随模型大小呈系统性变化。

（2）**低精度数据类型的量化规律**：不同低精度数据类型（如 MX 格式）的量化误差（以信号量化噪声比 SQNR 衡量）在模型家族和大小间相对稳定，但 Next token的NLL （负对数似然损失）损失呈现非线性模式，低精度格式通常导致更高损失；相比传统整数量化，MX 格式的量化效果更可预测。

（3）**PTQ 优化算法的效果规律**：PTQ 优化算法（如 GPTQ）的效果因模型大小、量化精度而异，在特定 SNR 窗口内效果显著，且对部分精度（如 mxint3_128）提升明显，其效果虽非单调但可通过局部损失解释。

（4）**预测模型的泛化法则**：结合预训练损失、局部损失、数据类型和 PTQ 算法等因素构建的随机森林预测模型，可准确预测量化后的 NLL 损失，针对不同模型家族、大小和数据集的预测具有良好泛化性。

<h1 id='88.什么是推理Scaling法则？'>88.什么是推理Scaling法则？</h1>

- 论文原文：[Inference-Time Scaling for Generalist Reward Modeling](https://arxiv.org/abs/2504.02495)

通过**动态调整奖励机制**来提升**模型的推理能力**，而非传统的通过增加模型参数或训练数据。推理能力的提升，使AI能够针对复杂、开放式问题给出更合理、更有帮助、更准确的答案。为实现有效的推理时扩展，采用策略有：

（1）**并行采样**：通过并行采样生成多个奖励集合，然后聚合得到最终奖励。大规模的采样可以准确地判断具有高多样性的原则，并输出细粒度的奖励。

（2）**自我原则批评（SPCT）**：在冷启动阶段实施拒绝式微调，奠定奖励模型基础能力；并通过基于规则的在线强化学习持续优化评估准则，从而增强奖励模型的泛化生成能力，最终实现推理阶段的可扩展性提升。

（3）**元奖励模型（meta RM）**：通过过滤低质量样本，仅保留顶部高质量样本参与投票，有效提升了扩展的稳定性和效果。

实验表明，这种推理时扩展策略效果显著，在多个基准测试中超过现有方法，甚至优于通过增大模型规模实现的训练时扩展，展现出推理计算量增加对性能提升的有效性。

<h1 id='什么是提示词压缩（PromptCompression）？'>88.什么是推理Scaling法则？</h1>

提示词压缩（Prompt Compression）是一项关键技术，它通过精炼提示词中的关键信息，使得每个输入的词元都承载更多价值，从而提升模型效率并还能控制成本。该技术核心是通过某种方式，构造一种专属于LLMs的讲言。比如：[LLMLingua](https://github.com/microsoft/LLMLingua)、[LongLLMLingua](https://aka.ms/LongLLMLingua)和[LongLLMLingua2](https://aclanthology.org/2024.findings-acl.57.pdf)。

![](./imgs/基础知识/LLMLingua.png)

<h1 id='90.什么是混合搜索？'>90.什么是混合搜索？</h1>

混合搜索在进行相似度搜索时，结合了两种核心方式：**稀疏检索**（sparse retrieval）和 **稠密检索**（dense retrieval）。稀疏检索的代表方法包括基于关键词统计的 TF-IDF 和 BM25，而稠密检索的代表则是将文本转化为语义向量的 embedding 向量化索引。稠密检索更侧重于理解查询（query）和文本片段（chunks）之间的深层语义关联，但其对语义的专注可能导致一个显著弱点：**当文本片段包含非常明显的关键词时，如果其整体语义表达与查询的语义向量匹配度不高，就可能被遗漏**，表现出稳健性相对较弱的特点。混合检索通过**集成（ensemble）稀疏检索方式**，有效地弥补了这一不足。

![](./imgs/基础知识/混合搜索.png)

<h1 id='91.什么是融合搜索？'>91.什么是融合搜索？</h1>

RAG 融合（rag fusion），主要思想是在Multi Query的基础上，对其检索结果进行重新排序，输出Top k个最相关文档，最后将这Tok k个文档喂给LLM并生成最终的答案。

![](./imgs/基础知识/融合搜索.png)

其中，排序包含两个动作，如图所示。

一是独立对每次检索返回的内容根据相似度排序，确定每个返回chunk在各自候选集中的位置，相似度越高排名越靠前，这一步采用rerank技术为主；二是对所有检索返回的内容利用RRF综合排序。


![](./imgs/基础知识/融合搜索-排序.png)
